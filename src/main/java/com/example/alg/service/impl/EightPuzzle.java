...
public class EightPuzzle implements Comparable {
    ...
    public List printRoute() {
        ...
        for (int i = answer.size() - 1; i >= 0; i--) {
            list.add(answer.get(i).print().get(0));
        }
        return list;
    }
    ...
    public AlgRepDTO solve(int stnum[], int tanum[]) {
        AlgRepDTO<List> algRepDTO = new AlgRepDTO();
        //定义open表
        ...
        if (start.isSolvable(target)) {
            ...
            while (open.isEmpty() == false) {
                ...
                close.forEach(par -> {
                    ...
                    o.setMisposition(par.getMisposition());
                    closeList2.add(o);
                });
                closeList.add(closeList2);
                if (best.isTarget(target)) {
                    System.out.println(cc);
                    //输出
                    algRepDTO.setList(best.printRoute());
                    long end = System.currentTimeMillis();
                    System.out.println("程序运行 " + (end - startTime) + " ms");
                    algRepDTO.setFlag(true);
                    algRepDTO.setOpenlist(openList);
                    algRepDTO.setCloselist(closeList);
                    return algRepDTO;
                }
                int move;
